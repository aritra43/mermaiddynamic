...
def generate_srs(uploaded_brd, original_filename="brd"):
        ...
        introduction_agent = Agent(
            ...
            goal="Write the Introduction, Purpose, Scope : in-scope and out-of-scope, and Overview sections in detailed paragraphs based on given brd.",
        ...
        cover_page_task = Task(
            description="Using given brd, generate the SRS Cover Page in a Word-style tabular format and a hyperlinked Table of Contents (ToC) using a proper layout for all main and sub-sections.",
            expected_output="Formatted cover page in word-style table and linked ToC.",
            agent=cover_toc_agent
        )

        intro_task = Task(
            description="Using given brd, write the Introduction section, covering Purpose, Scope: In-Scope, Out-of-Scope, Overview in detailed paragraphs. Definitions in Word-style table, and References in Word-style table. Use explanatory paragraphs and word-style tables where required.",
        ...
        functional_task = Task(
            description="From given brd, extract all functional requirements. Present each use case with descriptive text, use case tables in Word-style tabular format, and generate Mermaid diagram code blocks. Use node IDs to define shapes (e.g., A(Rounded), B[Square]), not inline shape wrappers in connections. Include headings before each diagram and specify correct layout (graph TD / graph LR) and styling.",
        ...
        nonfunctional_task = Task(
            description="List and describe all non-functional requirements like Performance, Reliability, Usability, etc. Provide brief explanatory paragraphs and organize content in Word-style tabular tables under each category from the given brd.",
        ...
        interface_task = Task(
            description="From the given brd, extract and describe all interfaces (User, Hardware, Software, Communication). Present each using Word-style tables with descriptions and interaction details.",
            expected_output="External Interfaces section organized by type with structured Word-style tables.",
            agent=interface_agent
        )

        datamodel_task = Task(
            description="Identify data entities from given brd. Generate a Mermaid ER diagram using proper layout, diagram type, and styling. Use headings before the code block. Ensure node shape declarations use ID-based syntax instead of inline shape brackets. Provide Word-style tables listing entities, attributes, data types, and constraints.",
        ...
        schema_task = Task(
            description="Create database schema section from given brd. Include Word-style tabular tables for all tables, listing fields, data types, keys, and constraints. Write brief paragraphs to describe the purpose of each table before the table.",
            expected_output="Database Schema section with annotated Word-style tables.",
            agent=schema_agent
        )

        assumption_task = Task(
            description="From given brd, list all business and technical assumptions, dependencies, third-party services, integration points, and known constraints. Use narrative explanation followed by Word-style tabular summaries wherever applicable.",
...
generate_srs(uploaded_brd, original_filename="brd")
